...
def test_legacy_cone_volume_model():
    ...
    # continuity at the cone/cylinder join
    assert abs(V(f - 1e-6) - V(f + 1e-6)) < 1e-3
    # monotonic
    vs = [V(i / 200) for i in range(201)]
    assert all(b >= a - 1e-9 for a, b in zip(vs, vs[1:]))
    # the conical bottom holds less than the linear approximation
    assert V(f) < f * C
    # slower fill near the bottom than near the top
    assert (V(0.05) - V(0.0)) < (V(1.0) - V(0.95))
    # cone_frac = 0 reduces to linear
