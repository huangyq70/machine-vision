...
#Serial Communication Port location (most likely somthing like 'dev/ttyUSB0')
RS232_PORT = ''
...
def process_frame(frame, detector, vol_history=None):
    ...
    #inintlizer volume variable to be returned to machine
    current_volume = 0
    ...
    return np.hstack(side_panels), current_volume
...
def main():
    ...
    # --- 2. Serial Communication Initialization ---
    serial_port = None
    try:
        serial_port = serial.Serial(RS232_PORT, BAUD_RATE, timeout=0)
        print("Serial Port Openend Successfully")
    except serial.SerialException as e:
        print("Error in serial initialization: {e}")
    ...
    # State variables for the comunication protocol
    auto_print_enabled = False
    last_auto_print_time = time.time()
    auto_print_interval = 0.5 # Seconds between continuous updates
    ...
    while True:
        ...
        result_frame, current_volume = process_frame(frame, detector, vol_history)
        cv2.imshow(window_name, result_frame)
        
        # --- Serial Communication Logic ---
        if serial_port and serial_port.in_waiting > 0:
            try:
                incoming = serial_port.read(serial_port.in_waiting).decode('ascii')

                if ("autoPrint") in incoming:
                    auto_print_enabled = True
                    print("recevied communication for turning on autopprint")
                elif ("stopPrint") in incoming:
                    auto_print_enabled = False
                    print("Received communication to trun off autoprint")
                elif ("print") in incoming and not auto_print_enabled:
                    response = f"{current_volume:.1f}mL\r\n"
                    serial_port.write(response.encode('ascii'))
            except UnicodeDecodeError:
                pass #anything that is gargabe within the buffer we just choose to ignore
        
        if auto_print_enabled:
            current_time = time.time()
            if (current_time - last_auto_print_time > auto_print_interval):
                response = f"{current_volume:.1f}mL\r\n"
                serial_port.write(response.encode('ascii'))
                last_auto_print_time = current_time
